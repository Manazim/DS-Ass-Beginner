...
public class Navigation{
    public static void main(String[] args) {
        ...
        Scanner sc = new Scanner(System.in); // to read line
        ...
        for (int i = 1; i <= number1; i++) {
            ...
            // add both location to LinkedList in GraphMeow
        ...
        for (int i = 1; i <= number2; i++) {
            ...
            if (jalan == null) {
                ...
                for (int j = jalan.size() - 1; j >= 0; j--) {
                    if (j == 0) {
                        Answer.append(graph.getAllLocationObjects().get(jalan.get(j)));
                        continue;
                    }
                    Answer.append(graph.getAllLocationObjects().get(jalan.get(j))).append("->");
                }
    ...
    private static LinkedList<Integer> printShortestDistance(ArrayList<ArrayList<Integer>> adj, int src, int dest,
            int v) {
    ...
    private static boolean BFS(ArrayList<ArrayList<Integer>> adj, int src, int dest, int v, int pred[], int dist[]) {
        ...
        for (int i = 0; i < v; i++) {
            visited[i] = false;
            // Set every dist to be infinite
            dist[i] = Integer.MAX_VALUE;
            // for string should be null
            pred[i] = -1;
        }
...
class Location<T extends Comparable<T>> extends ArrayList<T> {
    ...
    ArrayList<Location<T>> neighbourLocation;
    boolean visited;
...
class GraphMeow<T extends Comparable<T>> extends ArrayList<T> {
    Location<T> head;
    int size;
    ...
    public boolean addLocation(int num, T v) {
        ...
        // location already exist
            ...
            Location<T> newlocation = new Location<>(num, v, null);// set location v as new location (*total number of
                                                                   // location, *location name, *ArrayList for line)
            ...
            if (head == null) {
                ...
                Location<T> previous = head;// set another variable for set the nextLocation of the last element in the
                                            // list
            ...
            if (sourceLocation.locationInfo.compareTo(source) == 0) {
                ...
                while (destinationLocation != null) {
                    ...
                    if (destinationLocation.locationInfo.compareTo(destination) == 0) {
                        ...
                        // sourceLocation.neighbourLocation.add(destinationLocation); //add location to
                        // arraylist
    ...
    // method to print edge for each location but using ArrayList<E> declared in
    // Location
    ...
    public Location<T> getLocation(T v) {
        // if list == null
        if (head == null)
            return null;
